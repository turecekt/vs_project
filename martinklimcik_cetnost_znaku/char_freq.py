...
def get_input() -> List[AnyStr]:
    ...
    if len(sys.argv) > 1:
        ...
        print('Input text to parse. May be multiple lines. '
...
def char_frequency(text: str) -> None:
    """
    Count and print out stats about input text.

    Any non-english-alphabet characters will be ignored in stats
    ...
    if len(counts) == 0:
        print('Inputted text does not contain any tracked characters. '
              'No stats to show')
        return
    ...
    min_chars = [ch for ch, count in alphabet.items() if count == min_count]
    max_chars = [ch for ch, count in alphabet.items() if count == max_count]

    def grammar_case(number: int) -> str:
        """
        Add 's' to make plural of nouns.
    ...
    # print out stats
    print()
    print(f"The text contains {total_count} "
          f"character{grammar_case(total_count)}.")
    print(f"There {'is' if individual_chars == 1 else 'are'} "
          f"{individual_chars} individual "
          f"character{grammar_case(individual_chars)} from the alphabet, "
          f"average count per character being {average:.2f}")
    print(f"Least frequent character{grammar_case(len(min_chars))} "
          f"with {min_count} occurrence{grammar_case(min_count)}: "
          f"{', '.join(min_chars)}")
    print(f"Most frequent character{grammar_case(len(max_chars))} "
          f"with {max_count} occurrence{grammar_case(max_count)}: "
          f"{', '.join(max_chars)}")
