...
"""
    ...
    Delenie od 2 do sqrt(n): Táto metóda zahŕňa delenie čísla n od 2 až do jeho 
    druhej odmocniny. Ak sa nedá deliť žiadnym číslom v tomto rozsahu,
...
Miller-Rabin test funguje tak, že pre zadané číslo n, 
prvý krok je rozklad n-1 na d*2^r. Potom sa vyberie náhodné číslo a, 
ktoré sa zvolí z intervalu od 2 do n-2. Ďalej sa vykoná výpočet 
a^d mod n. Ak sa výsledok rovná 1 alebo n-1, 
číslo sa považuje za možné prvočíslo a test sa opakuje s iným číslom a. 
...
def prvocislo_mr(n, k=5):
    ...
    while d % 2 == 0:
        r += 1
        d //= 2
    # Pomocou for cyklu vykonáme 'k' iterácií Miller-Rabin testu s náhodnými číslami od 2 do n-2.
    for _ in range(k):
...
while True:
    # Vstup čísla od používateľa a jeho ošetrienie aby bolo číslo zadané užívateľom kladné celé číslo.
    try:
...
# Ak je číslo väčšie ako 1000 zavolá funkciu 'prvocislo_mr', čiže heuristickú metódu Miller-Rabin test.
if cislo > 1000:
