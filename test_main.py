...
@pytest.mark.parametrize("expected_exception, divider", [(TypeError, True),
                                                         (TypeError, 5 + 6j),
                                                         (TypeError, "ahoj")])
def test_char_min_values(expected_exception, divider):
    """Test function minimum values of chars."""
    assert char_min_values(dict_test) == ["b", "c", "1"]
    assert char_min_values({"a": 1}) == ["a"]
    with pytest.raises(expected_exception):
        char_min_values(divider)


@pytest.mark.parametrize("expected_exception, divider", [(TypeError, True),
                                                         (TypeError, 5 + 6j),
                                                         (TypeError, "ahoj")])
def test_char_max_values(expected_exception, divider):
...
@pytest.mark.parametrize("expected_exception, divider", [(TypeError, True),
                                                         (TypeError, 5 + 6j),
                                                         (TypeError, "ahoj")])
def test_char_average(expected_exception, divider):
    """Test function average value of chars."""
    assert char_average(dict_test) == 2.00
    assert char_average({"a": 1}) == 1

    with pytest.raises(expected_exception):
        char_average(divider)


@pytest.mark.parametrize("expected_exception, divider", [(TypeError, True),
                                                         (TypeError, 5 + 6j),
                                                         (TypeError, "ahoj")])
def test_occurrence_to_alphabetic_dict(expected_exception, divider):
    ...
    with pytest.raises(TypeError):
        occurrence_to_alphabetic_dict(divider)
