...
float: For divisiom operation if a % b != 0
...
for x in range(5):
    ...
    op = random.randint(0,3)    # Generates random number op <0, 3>, used to switch between operations

    while(b == 0):  # preventing division by 0
        b = random.randint(-10,10)

    if(b < 0):  
        b_string = "(" + str(b) + ")"   # it is a convension to display a negative number in brackets e.g. 4+(-3) 
    else:
    ...
    if(answer == str(count(op, a, b))): 
        correct += 1    # if the user input is a correct solution, counter of correct asnwers is increased
...
print(round(elapsed_time/5,2), end = '')    # print average elapsed time rounded to two decimal places
print(" seconds.")